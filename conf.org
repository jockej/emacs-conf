...
domain...
#+BEGIN_SRC emacs-lisp
  (defvar jj-home-host "jockej.mooo.com" "Hostname of my server")
#+END_SRC
...
I think this is more useful than the default "newline-and-indent" and "kill-line".
#+BEGIN_SRC emacs-lisp
  (defun jj-open-line-above ()
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "M-s i") 'jj-occur-this)
#+END_SRC

#+BEGIN_SRC emacs-lisp
  (require 'swiper-helm)
  (global-set-key (kbd "H-s") 'swiper-helm)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defvar jj-mail-update-interval 300 "Number of seconds between email checks")

  (if (jj-is-work-comp-p)
      (setq user-mail-address "joakim.jalap@tritech.se")
...
#+BEGIN_SRC emacs-lisp
  (if (display-graphic-p)
    ...
    (setq mu4e-view-show-images f
          mu4e-view-prefer-html f
          mu4e-use-fancy-chars f))
...
#+BEGIN_SRC emacs-lisp
  (require 'gnus-sync)
  (setq gnus-sync-backend (concat "/ssh:joakim@" jj-home-host
                                  "#20022:gnus-sync/gnus")
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "H-f") 'jj-fold-hydra/body)
...
- GNU global
- Excuberant ctags
  I use =universal-ctags-git= from AUR.
- libclang
- cscope
- ag, a.k.a. the silver searcher
- zsh
- mu
  I use =mu-git= from AUR.
- pyenv
  I only use this at work...
- gnuplot
- conkeror
- w3m
- html2text
